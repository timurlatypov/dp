...
class BrandController extends Controller
{
    ...
    {
        if (!$product = $brand->products()->where('slug', $product->slug)->live()->stock()->first()) {
            abort(404);
        }
    ...
    {
        $products = $brand->products()->orderBy('title_eng', 'asc')->live()->stock()->paginate(20);
    ...
    {
        $products = $line->products()->orderBy('order_id', 'asc')->live()->stock()->paginate(20);
