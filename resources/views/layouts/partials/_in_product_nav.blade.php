<div class="ml-2">
    ...
    @if(str_contains(url()->current(), 'category/direct-care'))
        <h4 class="title mt-0 mb-3"><nobr>Направленный уход</nobr></h4>
        <ul class="list-group py-0 pb-5">
            @foreach($direct_care->subcategories as $subcategory)
                <li class="list-group-item py-0">
                    <a class="btn btn-sm text-left {{ request()->is('category/direct-care/'.$subcategory->slug) ? 'btn-primary text-light font-weight-bold' : 'btn-white text-dark font-weight-bold' }}" href="{{ route('show.category.subcategory', [$direct_care, $subcategory]) }}">{{ $subcategory->name }}</a></li>
            @endforeach
        </ul>
    @endif
</div>