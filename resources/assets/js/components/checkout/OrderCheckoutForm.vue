      ...
      <table class="table table-shopping">
        ...
        <tr v-for="(item, index) in order.cart">
          ...
          <td class="td-number font-weight-bold">
            <nobr>{{ item.subtotal.toFixed(decimals) }}&nbsp;<i class="fas fa-ruble-sign" style="font-size: 90%"></i>
            </nobr>
          </td>
...
export default {
  ...
  methods: {
    ...
    calcTotal() {
      ...
      this.order.billing_original_total = 0;
      ...
      this.order.cart.forEach(function (product, index) {
        that.order.billing_original_total += Number(product.total);
      });
