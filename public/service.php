...
class ISDEKservice
{
    ...
    {
        self::toAnswer(['LANG' => self::getLangArray()]);
        self::printAnswer();
    }
    ...
    public static function calc($data)
    {
        ...
        if ($data['shipment']['cityToId']) {
            ...
            self::toAnswer(['error' => 'City to not found']);
        }
        ...
        if ($data['city']) {
            ...
            if ($result && $result['code'] == 200) {
                ...
                if (!isset($result->geonames)) {
                    self::toAnswer(['error' => 'No cities found']);
                } else {
                    self::toAnswer([
                ...
                self::toAnswer(['error' => 'Wrong answer code from server : ' . $result['code']]);
            }
        } else {
            self::toAnswer(['error' => 'No city to search given']);
        }
    ...
    protected static function requestPVZ()
    {
        if (!function_exists('simplexml_load_string')) {
            self::toAnswer(['error' => 'No php simplexml-library installed on server']);
        ...
        if ($request && $request['code'] == 200) {
            ...
            foreach ($xml as $key => $val) {
                ...
                if (!is_array($val->OfficeImage)) {
                    $arToCheck = [['url' => (string)$val->OfficeImage['url']]];
                } else {
            ...
            self::toAnswer(['error' => 'Wrong answer code from server : ' . $request['code']]);
    ...
    protected static function calculate($shipment)
    {
        ...
        if ($result && $result['code'] == 200) {
            if (!is_null(json_decode($result['result']))) {
                ...
                self::toAnswer(['error' => 'Wrong server answer']);
    ...
    protected static function client($where, $data = false)
    {
        if (!function_exists('curl_init')) {
            self::toAnswer(['error' => 'No php CURL-library installed on server']);
